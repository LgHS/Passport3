// Neutral silhouette (128x128 PNG, 431 bytes) served by /avatars/[file] when an avatar hash matches
// no member (neither an uploaded photo nor initials to draw), so that URL always answers with an image. Embedded rather
// than read from static/, which isn't reliably on disk next to the server bundle.
export const DEFAULT_AVATAR_PNG = Buffer.from(
	'',
	'base64'
);
